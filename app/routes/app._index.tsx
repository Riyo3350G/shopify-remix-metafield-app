    ...
    try {
      const response = await fetch("/api/metafields", {
        ...
        body: JSON.stringify({
          ...
          productTitles: selectedProducts.map((product) => product.title),
      ...
      if (data.success) {
        setResMessage({ type: "success", text: data.message });
        setCustomFieldValue("");
        setSelectedProducts([]);
      } else {
        setResMessage({
    ...
    <Page title="Product Custom Metafields">
      <Layout>
        <Layout.Section>
          {resMessage && (
            <Banner
              title={resMessage.type === "success" ? "Success" : "Error"}
              tone={resMessage.type === "success" ? "success" : "critical"}
              onDismiss={() => setResMessage(null)}
            >
              <p>{resMessage.text}</p>
            </Banner>
          )}
